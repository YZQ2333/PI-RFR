...
class KnowledgeConsistentAttention(nn.Module):
    def __init__(self, patch_size = 3, propagate_size = 3, stride = 1):
        ...
        self.patch_size = patch_size#补丁大小1
        self.propagate_size = propagate_size#传播大小3
        self.stride = stride#1
        self.prop_kernels = None
        self.att_scores_prev = None
        self.masks_prev = None
        self.ratio = nn.Parameter(torch.ones(1))#nn.Parameter()将变量变为模型的一部分，可以随之优化
        
    def forward(self, foreground, masks):
        bz, nc, h, w = foreground.size()#batch,in_channel,h,w
        if masks.size(3) != foreground.size(3):
            masks = F.interpolate(masks, foreground.size()[2:])#将mask插值成foreground的大小
        background = foreground.clone()#.clone()复制新的值，detach()共享同一存储空间
        background = background
        conv_kernels_all = background.view(bz, nc, w * h, 1, 1)
        conv_kernels_all = conv_kernels_all.permute(0, 2, 1, 3, 4)#将对应的维度换位
        output_tensor = []
        att_score = []
        for i in range(bz):
            feature_map = foreground[i:i+1]#获取第i个tensor,会保留当前维度
            conv_kernels = conv_kernels_all[i] + 0.0000001#shape=(w*h,nc,1,1)
            norm_factor = torch.sum(conv_kernels**2, [1, 2, 3], keepdim = True)**0.5# 先平方再求和,然后开根号————将三个通道的数值相加
            # input:输入一个tensor
            # dim:要求和的维度，可以是一个列表
            # keepdim:求和之后这个dim的元素个数为１，所以要被去掉，如果要保留这个维度，则应当keepdim=True
            conv_kernels = conv_kernels/norm_factor#每个数字加上0.0000001然后乘以三分之根号三
            ...
            if self.propagate_size != 1:
                ...
                conv_result = F.avg_pool2d(conv_result, 3, 1, padding = 1)*9#（inpute,kernel_size,stride,padding）
            attention_scores = F.softmax(conv_result, dim = 1)#沿指定维度做softmax，该维度结果和为1
            ...
            att_score.append(attention_scores)#注意力分数
            feature_map = F.conv_transpose2d(attention_scores, conv_kernels, stride = 1, padding = self.patch_size//2)#反卷积操作
...
class AttentionModule(nn.Module):
    ...
    def __init__(self, inchannel, patch_size_list = [1], propagate_size_list = [3], stride_list = [1]):
        ...
        self.att = KnowledgeConsistentAttention(patch_size_list[0], propagate_size_list[0], stride_list[0])#（1，3，1）
        self.num_of_modules = len(patch_size_list)#1
