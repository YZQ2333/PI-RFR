...
class PartialConv2d(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        ...
        #滑动窗口尺寸
        self.slide_winsize = self.weight_maskUpdater.shape[1] * self.weight_maskUpdater.shape[2] * self.weight_maskUpdater.shape[3]#卷积核大小
    ...
    def forward(self, input, mask=None):
        ...
        if mask is not None or self.last_size != (input.data.shape[2], input.data.shape[3]):
            ...
            with torch.no_grad():# 被该语句 wrap 起来的部分将不会track梯度
                ...
                self.update_mask = torch.clamp(self.update_mask, 0, 1)#将输入张量每个元素的夹紧到区间[min,max]，并返回结果到一个新张量sw
        ...
        raw_out = super(PartialConv2d, self).forward(torch.mul(input, mask) if mask is not None else input)#python3可以直接写super().xxx
